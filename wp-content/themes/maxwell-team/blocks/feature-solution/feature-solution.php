...
<section id="<?php echo esc_attr($anchor); ?>" class="<?php echo esc_attr($color_mode == 'dark' ? 'bg-section-dark' : ''); ?> relative overflow-hidden feature-solution-<?php echo esc_attr($blocks_id); ?> <?php echo esc_attr($blocks_class); ?>" <?php echo _padding($data['spacing']); ?>>
    <div <?php echo _padding($data['padding']); ?>>
        <div class="absolute inset-0 opacity-[0.03]" style="background-image: radial-gradient(circle at 1px 1px, <?php echo $color_mode == 'dark' ? 'rgb(255, 255, 255)' : 'rgb(0, 0, 0)'; ?> 1px, transparent 0px); background-size: 32px 32px;"></div>
            <div style="opacity: 1;">
                <div class="<?php echo _title_position($data['title_position'], $data['title_size']); ?>">
                    ...
                    <?php if ($data['text']): ?>
                        <div class="<?php echo esc_attr($color_mode == 'dark' ? 'text-white' : 'text-muted-foreground'); ?> text-lg text-center max-w-2xl mx-auto mb-16"><?php echo apply_filters('the_content', $data['text']); ?></div>
                    <?php endif; ?>
                </div>
                        ...
                        <?php foreach ($data['solutions'] as $solution): ?>
                            <div class="p-8 rounded-2xl <?php echo esc_attr($color_mode == 'dark' ? 'bg-white/5 border-white/10 hover:border-accent/50' : 'bg-card/50 border-border hover:border-primary/50'); ?> <?php echo $layout == 'center' ? " flex flex-col items-center justify-center text-center " : "" ?> border transition-colors group">
                                ...
                                <?php if ($solution['title']): ?>
                                    <h3 class=" font-bold <?php echo $box_title == "small" ? 'text-xl' : 'text-3xl'; ?> <?php echo esc_attr($color_mode == 'dark' ? 'text-white' : 'text-gradient'); ?> mb-3">
